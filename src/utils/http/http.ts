...
export default {
     ...
     * @returns {AxiosPromise<any>}
     */
    post(url: string, data: any, auth = false) {
        if (auth) {
     ...
     * @returns {AxiosPromise<any>}
     */
    put(url: string, data: any, auth = false) {
        if (auth) {
     ...
     * @param auth 是否需要带登录信息
     */
    uploader(url: string, file: string | Blob, auth = false) {
